...
if (isset($_POST['submit'])) {
    if ($_POST['city'] == '') {
        echo "Empty input, please write a city name!";
    } else {
        $city = $_POST['city'];
        $data = file_get_contents("https://api.openweathermap.org/data/2.5/weather?q=".
            $city ."&appid=a8595203cb5893d82db518976a9444fa");
        $weather = json_decode($data, true);
//        print_r($weather['cod']);

        $tempInCel = intval($weather['main']['temp'] - 273);
        $fullWeather = "The weather in " . $city . " is " . $weather['weather'][0]['main'] .
                       " and temperature is " . $tempInCel;
    }
}
        ...
        <div class="conatiner">
            <div class="row justify-content-center">
                <div class="col-md-6">
                    ...
                    <?php if ($fullWeather) : ?>
                        <div class="alert alert-success bg-success text-white"><?= $fullWeather ?></div>
                    <?php endif; ?>
                </div>
           </div>
        </div>
